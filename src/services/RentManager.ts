...
export class RentManager {
   ...
   */
  static chargeRent(gameState: GameState): {
    state: GameState;
    event: RentEvent;
  } {
    ...
    if (ownerIndex === -1) {
      throw new RentManagerError(
        `Owner ${currentTile.ownerId} not found in game`
      );
    }
   ...
   */
  static calcPropertyRent(houseRent: Map<string, number> | undefined): number {
