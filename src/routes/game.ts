 ...
 * /api/game/pay-rent:
 *   post:
 *     summary: Pay rent on another player's property
 *     description: Pay rent when the current player has landed on a property owned by another player. RoomCode is retrieved from user token (cookie). Player must be in PAY_RENT phase.
 *     tags: [Game]
 *     security:
 *       - cookieAuth: []
 *     responses:
 ...
 *                       example: true
 *       400:
 *         description: Missing required properties in token or rent charge failed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 message:
 *                   type: string
 *                   example: "Tile is not owned by another player"
 *       403:
 *         description: Not player's turn or wrong game phase
 *       404:
 *         description: Game not found
 ...
 *         description: Internal server error
 */
router.post('/pay-rent', requireAuth, GameController.payRent);
