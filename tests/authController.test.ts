...
describe('Auth Controller', () => {
  ...
  describe('POST /api/auth/userIdExists', () => {
    ...
    it('should return 400 when userId is too short', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Validation failed');
      expect(response.body.message).toBe('UserId must be at least 3 characters long');
    });
    ...
    it('should return 400 when userId has invalid characters', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Validation failed');
      expect(response.body.message).toBe('UserId can only contain letters, numbers, underscores, and hyphens');
    });
  });
  ...
  describe('POST /api/auth/register', () => {
    it('should register a new user successfully', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toBe('User registered successfully. Please login to continue.');
    ...
    it('should return 400 when required fields are missing', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Validation failed');
      expect(response.body.message).toBe('Username, password, and userId are required');
    });
    ...
    it('should return 400 when fields are not strings', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Validation failed');
      expect(response.body.message).toBe('Username, password, and userId must be strings');
    });
  ...
  describe('GET /api/auth/profile', () => {
    it('should return 501 (not implemented)', async () => {
      ...
      expect(response.body.message).toBe('Profile endpoint will be implemented later');
    });
  });
